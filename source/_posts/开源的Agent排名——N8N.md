...
大家好，我是牛皮糖！最近有一个奇思妙想的想法，把自媒体上说的很火的开源 Agent 做一个排名测试 ， 比如说从安装部署、对于国内友好度、以及小白的使用，当然有可能很多我安装不成功 ， 我就直接给出垃圾评价，所以能够安装成功的才能得分。直接给出 顶级 、一般 、NPC 、拉完了 四个级别。 

今天测试的是 N8N ， 这个一个已经在 GitHub 超过 100 k Stars ，开源数上
...
N8N 的安装相对很简单 ， 可以通过 node 直接安装 ，相比起来docker 安装。 npn 安装 更加简单，这一波也直接给到了顶级。

### 快速上手教程
n8n 的安装非常简单，以下是两种常用方式：

**方式一：使用 npx（需 Node.js）**
...
在国人友好度来说只能说一般， 全是英文需要进行对外的翻译，使用上需要自己配置各种 API 接口，国内食用度 NPC 。但是不缺乏一些极客大佬来接入一些国外的 API 接口，毕竟 n8n 的生态没话说和上一个 langflow 比起来强。
...
- • **可视化界面**：通过拖拽节点轻松构建工作流，无需编写代码。

- • **自定义代码**：支持 JavaScript/Python，甚至可以引入 npm 包，满足高级开发需求。
...
- • 支持 **400+ 应用和服务**，包括 Slack、Google Sheets、GitHub、Notion 等。
