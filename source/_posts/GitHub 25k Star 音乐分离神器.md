...
使用基于U-net的CNN模型进行人声和伴奏的分离，训练在Deezer的内部数据集上。它在musdb18数据集上表现出色，与Open-Unmix和Demucs相比，具有可比的分离性能，如SDR、SAR、SIR和ISR指标。Spleeter在单个GPU上运行速度快，能在2分钟内处理3小时音频，非常适合处理大型数据集。
