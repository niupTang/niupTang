---
title: GitHub 开源的高赞两款AI 声音克隆
date: '2026-01-04T16:26:13.000455'
updated: '2026-01-05T23:18:10.264833'
tags: []
categories: []
cover: null
---

* 戳上方蓝字�?*牛皮糖不吹牛**”关注我

    大家好，我是牛皮糖！�?GitHub 有两个有趣的项目，AI拟声: 5秒内克隆您的声音并生成任意语音内容�?

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrSLjMrZtJM9m5Os4SoIEe5b0hPo7G23gYoic1Fia3SYwLX9oeEqlQmwC8SRc8w9d2b5VnkSMbARR5vw/640?wx_fmt=png&from=appmsg)

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrSLjMrZtJM9m5Os4SoIEe5bgLEhhsqVTMCuqIzd70udVXDnjYrTzVhpO0oD1gfhgW0sAhbjjdsKPQ/640?wx_fmt=png&from=appmsg)

### Real-Time-Voice-Cloning

- GitHub 链接：https://github.com/CorentinJ/Real-Time-Voice-Cloning

- 主要功能：这个项目能够克隆一个人的声音并在实时中生成任意语音。它通过录制一个人的声音样本，然后使用这些样本来生成新的语音�?

- 技术特点：

- 克隆声音只需5秒钟的录音�?

- 实时生成语音�?

- 使用深度学习技术来建模声音特征�?

**
**

**
**

****

**![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrSLjMrZtJM9m5Os4SoIEe5b0hPo7G23gYoic1Fia3SYwLX9oeEqlQmwC8SRc8w9d2b5VnkSMbARR5vw/640?wx_fmt=png&from=appmsg)
**

### MockingBird

- GitHub 链接�?

https://github.com/babysor/MockingBird

- 主要功能：MockingBird 是一�?AI 拟声工具，能够在5秒内克隆声音并生成任意语音内容。它支持中文普通话，并且在多个数据集上进行了测试�?

- 技术特点：

- 支持中文，包括多个中文数据集�?

- 适用�?PyTorch，测试版本为 1.9.0�?

- 可在 Windows �?Linux 操作系统上运行，包括 M1 芯片�?macOS�?

- 提供了易于使用的合成器效果，通过重用预训练的编码�?声码器�?

- 支持 Web 服务器，可以远程调用生成结果�?

**
**

    Real-Time-Voice-Cloning 主要针对英文，而 MockingBird 特别支持中文普通话�?

    总的来说，两个项目都提供了声音克隆的能力，但 MockingBird 在中文支持、社区贡献和 Web 服务方面提供了更多的特性和便利。根据你的具体需求和偏好，你可以选择适合你的项目�?

![](https://mmbiz.qpic.cn/mmbiz_gif/7ibzJsmgW5wguO21SlkBAdxJgAicEOVCzDiaObyzEAEMTI527clib7gHvKfBtDu8MJZLwwEIVuVBmqfn01fmLDdTfQ/640?wx_fmt=gif&tp=webp&wxfrom=5&wx_lazy=1)

·················END·················

### **推荐阅读**
•�? [github 95.5k Star 的项目集合地](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485137&idx=1&sn=00da6656ecacfcab683c6e149c208985&chksm=c1d3a4d7f6a42dc107aa9233b7a23f20a4bfe7bcd9d48a0fe24787faae37539bd65663d8ba8e&scene=21#wechat_redirect)[‍](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)•�? [4�?16G 就能 RAGFlow Quick start 快速入门](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)•   [github 7.8k star 将小爱音箱接�?ChatGPT 和豆包，改造成你的专属语音助手。](http://mp.weixin.qq.com/s?__biz=MzIxODg1OTk1MA==&mid=2247488494&idx=1&sn=0244c0a45012f5a6ca6cdf9a0ac88024&chksm=97e5432fa092ca39991868bda959cfd8bafd949fa445ef72bdca5cc6c4dc2ed4bd51e22634f8&scene=21#wechat_redirect)

**
**

**点这里👇关注我，记得标星哦�?*

**一键三连「分享」、「点赞」和「在看�?*

**科技前沿进展日日相见 ~ **

![](https://mmbiz.qpic.cn/mmbiz_svg/g9RQicMD01M0tYoRQT2cMQRmPS5ZDyrrfzeksiay90KaDzlGBH61icqHxmgFKfvfXtVuwTHV740CDLAaXU1LIfZyoJEpYKcRIiaE/640?wx_fmt=svg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)