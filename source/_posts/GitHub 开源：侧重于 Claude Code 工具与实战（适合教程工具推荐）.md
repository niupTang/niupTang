...
大家好，我是牛皮糖！最近再看Claude Code 插件， 从Skills 到 Plugins 然后再是 Agents , 确实有很多对应的开源工具， 主打一个全面。

[GitHub 开源：把 Claude Code 打造成“六边形战士”](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247492005&idx=1&sn=82a975f9f3899f102637cdc0a81fa972&scene=21#wechat_redirect)
...
简单来说，它就像是为 Claude Code 这个“超级程序员”配备了一个庞大的**插件市场**和**专家团队**，通过高度模块化的设计来增强其处理复杂软件开发任务的能力。
...
- • 107 个智能体技能 (Agent Skills)：模块化的知识包，赋予智能体特定的专业能力。

- • 15 个工作流编排器 (Orchestrators)：用于协调多个智能体完成复杂任务（如全栈功能开发、安全加固）。
...
### 2. 核心设计理念
该项目为了解决 AI 编程中的 Token 消耗和上下文污染问题，采用了独特的设计架构：

- • **极简 Token 使用** (Minimal Token Usage)： 插件采用颗粒化设计，每个插件平均仅包含 3.4 个组件。当你安装一个插件时，它**只加载**相关的智能体和工具，不会把整个市场都塞进上下文里。

例如：安装 `python-development` 插件只会加载 3 个 Python 专家智能体和 5 个相关技能，消耗约 300 tokens。

- • **渐进式披露**(Progressive Disclosure)**：智能体的技能采用三层架构。Claude 最初只加载技能的“元数据”（名字和触发条件），只有在真正需要使用该技能时，才会加载具体的“指令”和“资源”。这种机制极大地节省了上下文窗口,。

- • **三层模型策略** (Three-Tier Model Strategy)： 为了平衡性能与成本，该系统针对 Opus 4.5、Sonnet 4.5 和 Haiku 4.5 进行了分层优化：

- • **Tier 1**Opus 4.5：处理关键架构设计、安全审计。

- • **Tier 3**Sonnet 4.5：处理常规文档、测试和调试。

- • **Tier 4**Haiku 4.5 ：处理快速的运维任务和内容生成,。

### 3. 主要功能与应用场景
通过安装不同的插件，你可以让 Claude Code 获得特定的能力：

- • **全栈开发编排**：使用 `/full-stack-orchestration` 命令，协调后端架构师、前端开发者和数据库专家共同完成一个功能。

- • **安全加固**：使用 `/security-scanning` 命令，进行多智能体的安全评估和漏洞扫描。

- • **基础设施运维**：包含 Kubernetes 操作（生成 Helm 图表、GitOps 配置）和云基础设施管理（AWS/Azure/GCP）,。

- • **语言与框架支持**：提供针对 Python、JavaScript/TypeScript 等语言的深度支持，包括异步模式、测试模式和包管理技能。

### 4. 如何使用
这个项目直接集成到 Claude Code 的 CLI 中：

- 1. **添加市场**： 在 Claude Code 中输入 `/plugin marketplace add wshobson/agents`，这会使所有 67 个插件变为可用状态（但暂不加载到上下文）。
