...
permalink: github-开源给你的-claude-code-加上海马�?
...
今天我们要介绍的这个 GitHub 开源项目，完美的解决了我们�?AI 编程上的记忆上的痛点。你们在使用 AI 编程助手（比如最近大火的 Claude Code 命令行工具）时，有没有遇到过这种**崩溃时刻**�?
...
为了解决一个复杂的架构拆分或者一个棘手的 Bug，你�?AI 一来一回聊了二百个回合，终于有了眉目，达成了共识。结果第二天早上准备开工，或者不小心关掉了终端窗口……一切重置�?

你面对着一个崭新的、一脸茫然的 AI，不得不像个复读机一样，把项目背景、技术栈约定、之前的进展重新“喂”给它。这不仅心累，还极其浪费时间，更重要的是—�?*非常烧钱**（每一次重复发送上下文，都在燃烧你�?API Token 啊！）�?

### 给你�?Claude 装上“海马体”：`claude-mem`
这个开源项目名叫 `claude-mem`�?
...
那么，`claude-mem` 就是给这�?CPU 外挂了一个“智能硬�?+ 智能索引系统”。它�?AI 不再是“用完即走”的快餐式服务，而是变成了能陪你长跑项目的伙伴�?

核心功能拆解我仔细研究了这个项目的实现逻辑，发现它并不是简单粗暴地“保存所有聊天记录”（那样Token开销依然巨大），它的智能体现在以下几个架构细节上�?

**1. 智能压缩与静默记�?(The Smart Scribe)** 它像一个尽职的书记员，在后台默默记�?Claude 的关键操作（比如运行了什么命令、修改了哪个核心文件）。最关键的是，它会利�?AI 能力把这些冗长的交互信息进行“脱水压缩”，提取精华要点存储下来，而不是傻傻地存一堆原始对话�?
...
**3. 交互式记忆检�?(The Librarian)** 这是一个非常强大的 Skill（技能）。你可以像问人类同事一样自然地�?Claude�?

“嘿，回顾一下我们上周关于用户登录模块的架构决定。�?“那个数据库连接超时�?Bug，我们上次排查到哪一步了？�?

Claude 会通过 `claude-mem` 提供的能力，自动检索记忆库并准确回答你。这比你自己去翻几千行的终端历史记录快了一万倍�?

保姆级使用教程`claude-mem` 是作�?Claude Code 的一个插件存在的，安装非常符合开发者的习惯，几行命令搞定�?
...
`claude plugin add thedotmack/claude-mem`（通俗解释：这条命令就是告�?Claude 的主程序，去 GitHub 上把这个叫“记忆模块”的小兄弟接回家。）
...
注意事项与补�? •�?*巨大的优势——省钱：** 这是重点！官方数据显示，通过其智能压缩和按需检索机制，相比于每次都全量发送巨大的历史记录，它每次会话启动能帮你节省大量的 Token 成本。长期做项目，省下来的钱非常可观�?
...
- •�?*适用范围�?* 它目前是深度绑定 Anthropic 官方的 `Claude Code` CLI 工具的，如果你用的是网页�?Claude 或者其他第三方 API 客户端，暂时无法享用这个神器�?
