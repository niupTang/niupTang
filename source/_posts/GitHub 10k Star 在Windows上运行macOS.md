...
permalink: github-10k-star-windows-macos
...
`docker run -it --rm -p 8006:8006 --device=/dev/kvm --cap-add NET_ADMIN --stop-timeout 120 dockurr/macos`这行命令会启动一个macOS容器，并�?006端口映射到宿主机，让你可以通过Web浏览器访问macOS环境。此外，你还可以通过设置环境变量来选择不同的macOS版本，比如：
