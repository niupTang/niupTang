---
title: "GitHub 86.8k Stars 免费的SaaS 、PaaS 产品列表"
permalink: github-86-8k-stars-免费的saas-paas-产品列表
date: 2024-09-02 22:25:58
tags:
  - 公众号
  - 原创
categories:
  - 公众号文章
---

* 戳上方蓝字“**牛皮糖不吹牛**”关注我

    大家好，我是牛皮糖！今天在 GitHub 看到一个开源的的 DevOps 和 Infradev 感兴趣的免费的 SaaS、PaaS 和 IaaS 产品列表。

    这些都是提供免费的服务，这些服务列表不仅仅是免费的，而且多数都是有一年的而且安全在线服务。

**概述**

**   **

服务类别：列表中包含了多种类型的服务，如自动化工作流（Actions）、代码托管和管理（Packages）、安全服务（Security）、即时开发环境（Codespaces）、代码辅助编写（GitHub Copilot）、代码审查（Code review）、问题跟踪（Issues）、讨论区（Discussions）等。

资源和学习材料：项目还提供了学习路径、白皮书、电子书、网络研讨会和客户故事等资源，以及合作伙伴信息。

定价信息：虽然项目名为 "free-for-dev"，但也提供了关于这些服务的定价信息，方便用户了解在超出免费套餐后可能需要支付的费用。

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrQuDhNh26icVibHTuWGbSfQmX9NJSvsFF4lpKLGibfhxBOeon2icu8iclb9K3sDHqzABFDYWGmGicLgkXIQ/640?wx_fmt=png&from=appmsg)

****

# **目录**
- # 主要云服务的免费限制

- 云管理解决方案

- 分析、事件和统计

- API、数据和机器学习

- 工件存储库

- 基础即服务 (BaaS)

- 低代码平台

- CDN 和保护

- CI 和 CD

- 内容管理系统

- 代码生成

- 代码质量

- 代码搜索和浏览

- 崩溃和异常处理

- 地图上的数据可视化

- 托管数据服务

- 设计和用户界面

- 设计灵感

- 开发博客网站

- DNS

- Docker相关

- 领域

- 教育和职业发展

- 电子邮件

- 功能切换管理平台

- 字体

- 表格

- 生成式人工智能

- 基础设施即服务 (IaaS)

- IDE 和代码编辑

- 国际手机号码验证API及SDK

- 问题跟踪和项目管理

- 日志管理

- 移动应用程序分发和反馈

- 管理系统

- 消息传递和流媒体

- 各种各样的

- 监控

- 平台即服务 (PaaS)

- 包构建系统

- 支付和账单整合

- 隐私管理

- 截图 API

- Flutter 相关以及无需 Mac 即可构建 IOS 应用

- 搜索

- 安全和 PKI

- 身份验证、授权和用户管理

- 源代码存储库

- 存储和媒体处理

- 隧道、WebRTC、Web 套接字服务器和其他路由器

- 测试

- 团队和协作工具

- 翻译管理

- Vagrant 相关

- 访客会话记录

- 网站托管

- 评论平台

- 基于浏览器的硬件模拟

- 远程桌面工具

- 游戏开发

- 其他免费资源

****

****![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrQuDhNh26icVibHTuWGbSfQmXUJ9x9lqP9NeiayB7WCgaO6fgwWkQN5lUmSEibLTafrQx4tnBOgBJQZYA/640?wx_fmt=png&from=appmsg)
****

**
**

**
**

项目地址：

**https://github.com/ripienaar/free-for-dev**

![](https://mmbiz.qpic.cn/mmbiz_gif/7ibzJsmgW5wguO21SlkBAdxJgAicEOVCzDiaObyzEAEMTI527clib7gHvKfBtDu8MJZLwwEIVuVBmqfn01fmLDdTfQ/640?wx_fmt=gif&tp=webp&wxfrom=5&wx_lazy=1)

·················END·················

### **推荐阅读**
•   [github 95.5k Star 的项目集合地](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485137&idx=1&sn=00da6656ecacfcab683c6e149c208985&chksm=c1d3a4d7f6a42dc107aa9233b7a23f20a4bfe7bcd9d48a0fe24787faae37539bd65663d8ba8e&scene=21#wechat_redirect)[‍](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)•   [4核 16G 就能 RAGFlow Quick start 快速入门](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)•   [github 7.8k star 将小爱音箱接入 ChatGPT 和豆包，改造成你的专属语音助手。](http://mp.weixin.qq.com/s?__biz=MzIxODg1OTk1MA==&mid=2247488494&idx=1&sn=0244c0a45012f5a6ca6cdf9a0ac88024&chksm=97e5432fa092ca39991868bda959cfd8bafd949fa445ef72bdca5cc6c4dc2ed4bd51e22634f8&scene=21#wechat_redirect)

**
**