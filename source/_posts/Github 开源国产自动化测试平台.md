...
    MeterSphere 一站式开源持续测试平，在GitHub 上面获取 11.4k Star。累计安装部�?757958 次�?
...
- 接口测试：集 Postman 的易用与 JMeter 的灵活于一体，接口调试、接口定义、接�?Mock、场景自动化、接口报告，你想要的都有�?

- 团队协作：采用“系�?组织-项目”分层设计理念，帮助用户摆脱单机测试工具的束缚，方便快捷地开展团队协作；

- 插件体系：提供各种类别的插件，用户可以按需取用，快速实�?MeterSphere 测试能力的扩展以及与 DevOps 流水线的集成�?
...
    不仅提供了线上安装环境、可以通过运维面板1Panel 应用商店 快速部�?MeterSphere�?
...
``# 用户�? admin``# 密码: metersphere`
也可以通过其他离线安装，安装需�?

组件默认端口说明MeterSphere8081MeterSphere 主服务项目，浏览器访问端口Task-Runner8000为接口测试提供独立节点类型的测试资源池MySQL3306默认安装的数据库对外提供的端口Redis6379默认安装�?Redis 对外提供的端口Minio9000�?001默认安装的分布式对象存储对外提供的端口、分布式对象存储控制台端口Kafka9092默认安装的消息中间件对外提供的端口�?
