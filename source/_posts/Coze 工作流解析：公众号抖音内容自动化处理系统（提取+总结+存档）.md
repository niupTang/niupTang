---
title: "Coze 工作流解析：公众�?抖音内容自动化处理系统（提取+总结+存档�?
permalink: coze-工作流解�?公众号抖音内容自动化处理系统-提取-总结-存档
...
[只需三步——帮我妈拿下最强微�?AI 助手](https://mp.weixin.qq.com/s?__biz=MzU5MzcwNDk0OA==&mid=2247487034&idx=1&sn=22fb7eae7e6f691ada59e1cfcb1fe17d&scene=21#wechat_redirect)
...
## 一、工作流核心架构总览
整个工作流由 **12个节�?13条数据链�?* 构成，按“输入层→处理层→输出层”三层逻辑划分，自动区分“公众号链接”和“抖音链接”并匹配对应处理流程，最终生成结构化飞书文档�?
...
输入�?
开始节点（100001�?
接收用户输入的“文�?视频链接”，定义核心输入参数URL
处理�?
链接识别�?41986）、选择器（115918）、内容提取（128311/110485）、文本处理（152537）、代码汇总（169071）、总结�?49038）、修复（185896）、信息合成（104914�?
完成“链接解析→平台分流→内容抓取→格式处理→质量优化�?
输出�?
飞书文档创建�?94958）、结束节点（900001�?
将处理后的内容存档至飞书，并返回结构化结�?文档链接
## 二、各节点功能拆解与数据链�?
### （一）输入层：开始节点（ID�?00001�?
...
- �?必选：`URL`（文�?视频链接，支持公众号文章链接、抖音视频链接）
...
### （二）处理层：核心逻辑与节点配�?
处理层是工作流的核心，通过“链接识别→平台分流→内容提取→内容优化�?步，实现多平台内容的统一处理�?

1. 第一步：链接识别（ID�?41986，大模型节点�?*功能**：从用户输入中提取纯净链接，并为后续“平台判断”提供依据�?
...
2. 第二步：平台分流（ID�?15918，选择器节点）**功能**：根据链接域名判断内容来源（公众�?抖音），匹配对应提取逻辑，实现“一分二”的分支处理�?
...
- •�?*关键价�?*：通过分支逻辑避免“一套提取规则适配多平台”的兼容性问题，提升内容提取准确率�?
...
3. 第三步：内容提取�?个插件节点，分平台处理）根据选择器的分支结果，分别调用对应插件提取内容，确保不同平台的内容格式适配�?
...
4. 第四步：内容整合（文本处�?代码节点）将不同平台提取的内容统一格式，避免后续“总结/修复”节点因格式差异出错�?
...
- �?输入：公众号整合文本�?52537输出）或抖音语音文案（`data.asr.text_srt`）；

- �?代码逻辑：通过Python代码将两种来源的内容合并为统一变量`key0`（避免分支数据分散）�?

- �?输出：`key0`（包含“标�?全文/文案”的标准化内容）�?
...
5. 第五步：内容优化�?个大模型节点，总结+修复）对统一格式的内容进行“价值提炼”和“质量优化”，提升内容可用性：
...
### （三）输出层：飞书存�?结果返回
1. 飞书文档创建（ID�?94958，飞书云文档插件�?*功能**：将结构化内容自动生成飞书文档，实现“处理→存档”无缝衔接�?

- •�?*输入参数**�?

- • `content`：信息合成节点的Markdown内容�?04914输出）；

- • `title`：固定标题“文案”（也可配置为“公众号-XXX标题”或“抖�?XXX标题”，需修改参数为`data.title`）�?

- •�?*输出结果**�?

- • `data.url`：飞书文档链接（可直接分享或存档）；

- • `data.token`：文档唯一标识（用于后续编�?管理）�?

2. 结果返回（ID�?00001，结束节点）**功能**：向用户输出最终结果，包含“结构化内容+飞书文档入口”，直观呈现处理成果�?
...
- •�?*关键设置**：开启“返回文本”模�?“Markdown渲染”，确保用户端能直接看到彩色标题、跳转链接和分层内容，体验更友好�?

## 三、工作流核心优势与适用场景
### 1. 核心优势
- •�?*全自动化无人工干�?*：从输入链接到生成飞书文档，无需手动复制、格式调整，全程1-2分钟完成�?

- •�?*多平台自适应**：自动区分公众号/抖音，匹配专属提取逻辑，避免内容提取不完整（如抖音语音转文字、公众号图片保留）；

- •�?*内容质量双优�?*：同时实现“核心总结”（提效）和“文案修复”（提质），输出内容可直接用于二次创作或存档�?

- •�?*企业级存�?*：对接飞书文档，支持团队协作分享、关键词检索，解决素材散存难管理的问题�?

### 2. 适用人群/场景
- •�?*新媒体运�?*：批量处理公众号干货文章、抖音热点视频，快速搭建素材库�?

- •�?*内容创作�?*：提取竞品内容核心观点，结合修复后的文案优化自身创作�?

- •�?*企业培训/行政**：整理行业报告（公众号长文）、员工教程（抖音视频），生成标准化文档存档；

- •�?*个人学习�?*：抓取知识类内容（如公众号科普、抖音技能教程），生成带总结的笔记，方便复习�?

AI 时代到来，要个体的能力加强，在自媒体时代下用  AI + 副业要这一切变得 。在当下最好发展一份属于自己的副业 [AI + 行业做副业](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247491231&idx=2&sn=bd93a7254853228d4bf8b47cb8c010df&scene=21#wechat_redirect) 已经�?4000 名小伙伴加入了，如果你也想着�?AI 时代拥有一份属于自己的 AI 副业 [戳链接](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247491231&idx=2&sn=bd93a7254853228d4bf8b47cb8c010df&scene=21#wechat_redirect) 加入吧！这是一个赚钱训练营，AI 技能训练营密集的圈子，你可以每年参加各种副业赚钱训练营�?
