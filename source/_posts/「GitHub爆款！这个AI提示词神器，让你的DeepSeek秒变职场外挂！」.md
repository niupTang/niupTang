---
title: "「GitHub爆款！这个AI提示词神器，让你的DeepSeek秒变职场外挂！」"
permalink: github爆款-这个ai提示词神器-让你的deepseek秒变职场外挂
date: 2025-03-31 20:42:10
tags:
  - 公众号
  - 原创
categories:
  - 公众号文章
---

* 戳上方蓝字“**牛皮糖不吹牛**”关注我

    大家好，我是牛皮糖！

**“别人的AI能写周报、做PPT，你的AI只会回‘好的’？**

最近发现，所有AI高手都在偷偷用同一个神器——**Prompt Optimizer**，它能让你的垃圾提示词秒变黄金指令！不用学任何技巧，AI自己就能帮自己优化！

——不会写提示词？让AI自己优化自己！

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrTuDgwEmawnM4RJiaiayvoX5ONaARvyCknlwKibHtUOQKpBkwmhWbS7Z97xrCttjot4XTIPGBRDrPxkg/640?wx_fmt=png&from=appmsg)

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrTuDgwEmawnM4RJiaiayvoX5OociajJut6JOgqCjsUSqaXy3jPibD6tibACg5QesHzMoq5gao7zQGiaCRfQ/640?wx_fmt=png&from=appmsg)

### **核心痛点：**
**🐾 铲屎官们，你还在这样浪费AI会员吗？**- • **场景1**：输入“写篇文章” → AI给你200字废话文学

- • **场景2**：手动调教半小时 → 结果下次提问又失效

- • **场景3**：换个大模型 → 之前积累的提示词全作废

- 

- ![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrTuDgwEmawnM4RJiaiayvoX5OhAzKNCloYBS1EavXuPibDWeicgpb3HKSFv6BXZlFShdI8bB7J7BVcEwQ/640?wx_fmt=png&from=appmsg)

**💡 而高手只需要：**

- 1. 输入“帮我写周报”

- 2. 点击【一键优化】

- 3. 获得完美指令：
“你是一名资深项目经理，请用Markdown格式生成包含：本周进度（数据量化）、卡点分析（分技术/协作）、下周计划（SMART原则）的周报”

### **神器能力拆解：**
**🔧 这不是工具，是AI的“自我修养”培训班！**

- • **自诊自疗**：自动识别模糊指令，补全专业术语（比如加上“用金字塔结构输出”）

- • **模型翻译官**：把给GPT-4的提示词自动适配Gemini/DeepSeek

- • **记忆大师**：保存成功提示词，建立你的专属《AI驯服手册》

**🛡️ 更放心的设计：**

- • 数据直连AI厂商 → 你的API密钥连开发者都看不到

- • 浏览器插件版 → 公司电脑也能偷偷用（摸鱼党狂喜）

### **3秒上手教程：**
**🐱 Docker党：**

`docker run -d -p 80:80 --name ai-butler linshen/prompt-optimizer`**🦊 Chrome党：**

- 1. 安装插件（文末获取）

- 2. 遇到AI智障时 → 点击图标 → 看它自我改造

### **结尾暴击：**
项目地址：https://github.com/linshenkx/prompt-optimizer
**⚠️ 重要提醒**：
本工具可能导致副作用——同事怀疑你突然变专业了！

![](https://mmbiz.qpic.cn/mmbiz_gif/7ibzJsmgW5wguO21SlkBAdxJgAicEOVCzDiaObyzEAEMTI527clib7gHvKfBtDu8MJZLwwEIVuVBmqfn01fmLDdTfQ/640?wx_fmt=gif&tp=webp&wxfrom=5&wx_lazy=1)

·················END·················

### **推荐阅读**
•   [Github 资料项目合集](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247488210&idx=1&sn=9b97cc6da4bdaf42ba779d4d0d66f5a1&scene=21#wechat_redirect)[‍](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)•   [4核 16G 就能 RAGFlow Quick start 快速入门](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)• [  github 7.8k star 将小爱音箱接入 ChatGPT 和豆包，改造成你的专属语音助手。](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485065&idx=1&sn=8b71c116b61add064e0892da63a7bf6d&scene=21#wechat_redirect)  

• [  ](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485065&idx=1&sn=8b71c116b61add064e0892da63a7bf6d&scene=21#wechat_redirect)[50个 AI 开源项目合集](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247489466&idx=1&sn=656577c7880b921ed675c55ba12c1c09&scene=21#wechat_redirect)