---
title: "别人几周才能做完的对话式 AI，我用 5 分钟上线了！"
permalink: 别人几周才能做完的对话式-ai-我用-5-分钟上线了
...
**Chainlit + LangChain，仅用 5 分钟，就构建了一个可直接上线的对话式 AI 应用。**

没错，不用前端，不用部署后台框架，甚至不需要搞复杂的 prompt 工程，**照着文档抄就能跑，效果还不**
...
- • **前端自动生成**：后台是 FastAPI WebSocket 服务，自动渲染 React 驱动的聊天 UI；

- • **工具集成 & 中间状态可视化**：支持自定义步骤、函数调用与 chain-of-thought 展示 (github.com, docs.chainlit.io)；

- • **多平台部署**：可作为独立 Web 应用、嵌入 FastAPI、Slack/Discord Bot 等 (chainlit.io)。

### ⚙️ 安装与快速上手
安装非常简单：

`pip install chainlit
chainlit hello`启用后会打开浏览器中的 demo 聊天 UI，非常直观 。
...
- • `@cl.on_chat_start`、`@cl.on_message`、`@cl.on_chat_end` 等钩子控制流程 (datacamp.com)；
...
@cl.on_chat_start
async def start():
    await cl.Message(content="📄 请上传一个 PDF 文件构建知识库").send()
...
- • ✅ 用 LlamaIndex 做更智能的索引摘要

- • ✅ 把 Chainlit 嵌入到你的 FastAPI 项目中
...
而 Chainlit 就是帮助你用最简单的方式，让“AI 动起来”的利器。

你可以 5 分钟部署一份 AI 产品原型，也可以 5 天内交付一整套企业级问答系统。
