...
大家好，我是牛皮糖！众所周知，DeepSeek 的多模态图片识别总是弱项，前面两天 DeepSeek 发布了自己 OCR 图片识别。它采用了一种突破性的“**上下文光学压缩**”技术路线，开源第二天就已经图片 10 k Stars了。
...
它**先将文本渲染成图像，然后通过高效的压缩算法将其转换为视觉令牌**，再由大语言模型进行处理。这种方法听起来有点绕，但实际效果惊人——**在单个A100-40G GPU上每天能处理超过20万页文档**，OCR准确率高达97%。

这种设计思路的巧妙之处在于，**将长篇文本压缩成视觉令牌，极大减轻了LLM的上下文负担**，让模型能够高效处理超长文档。

DeepSeek-OCR 采用 “编码器 - 解码器” 端到端架构，核心创新集中在**DeepEncoder（视觉编码器）** 和**MoE 解码器**的设计，同时通过多分辨率支持和多样化数据引擎提升实用性。
...
- • **多格式文档解析**：支持PDF、图片等多种格式的文档解析，保持原有布局和结构

- • **多语言支持**：对中文、英文等语言都有出色的识别效果
...
- • **处理速度**：相比传统OCR工具，处理长文档的速度提升**近10倍**

- • **准确率**：在复杂排版的中文文档中，文字识别准确率稳定在**97%以上**
...
- • **硬件要求**：虽然可以在消费级GPU上运行，但要发挥最佳性能建议使用**显存足够的显卡**

- • **模型大小**：完整的模型需要一定的磁盘空间，请确保有**足够的存储空间**

- • **依赖管理**：Python环境配置需要一定的技术基础，新手可能需要耐心排查依赖问题

- • **格式适应性**：对于极其古老或特殊排版的文档，识别效果可能会有一定折扣

## 总结
DeepSeek-OCR代表了文档处理的一个新方向，它通过**创新的“上下文光学压缩”技术**，巧妙地解决了长文档处理的痛点。无论是处理技术文档、学术论文还是商业报告，它都能提供**高效、准确的文字识别和格式转换服务**。
