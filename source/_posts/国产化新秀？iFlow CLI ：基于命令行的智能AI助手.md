...
大家好，我是牛皮糖！Claude 疯狂封禁�?国内工具猥琐发育�?本身 Claude Code 需要用墙，Claude 封禁再度加强。最近在GitHub �?爆火的国�?CLI 工具 iFlow 直接一波起飞。iFlow CLI 是一款由阿里旗下心流团队开发的终端 AI 助手工具，它允许开发者通过自然语言指令在命令行中执行各种任务，如代码分析、文档生成、文件管理等�?
...
iFlow CLI 是一款由阿里旗下心流团队开发的终端 AI 助手工具，它允许开发者通过自然语言指令在命令行中执行各种任务，如代码分析、文档生成、文件管理等。以下是基于搜索结果�?iFlow CLI �?GitHub 中文 README 的相关信息概要：

### 1. **核心功能**
- •�?*自然语言交互**：用户可以用日常语言描述需求，iFlow CLI 会理解并执行相应任务，如代码生成、Bug 修复、文件整理等 �?

- •�?*多模型支�?*：内置多种免�?AI 模型（如 Qwen3-Coder、Kimi K2、GLM-4.5、DeepSeek-V3 等），也支持兼容 OpenAI 协议的第三方模型 �?

- •�?*自动化任�?*：可自动化执行复杂工作流，例如分析代码库、生成文档、部�?Docker 项目（如自动处理 Paperless-ngx 部署）�?

- •�?*文件操作与系统命�?*：通过 `!` 执行 Shell 命令（如 `!ls -la`），或用 `@文件路径` 引用文件进行分析 �?

### 2. **安装与配�?*
- •�?*系统要求**：需�?Node.js 20+（推�?22+）和 4GB+ 内存 �?

- •�?*安装命令**�?

- •�?*macOS/Linux**：一键安装脚本：`bash -c "$(curl -fsSL https://cloud.iflow.cn/iflow-cli/install.sh)"`

- •�?*Windows**：需先安�?Node.js �?WSL（适用�?Linux �?Windows 子系统），然后通过 npm 安装：```
...
### ⚙️ 4. **进阶特�?*
- •�?*多模式运�?*：支�?Yolo（全自动）、Accepting Edits（仅允许文件修改）、Plan Mode（先规划后执行）等模式，可通过 `Shift + Tab` 切换 �?

- •�?*MCP 集成**：支持心�?MCP 市场扩展工具（如云效 Mcp Server），可连�?DevOps 工具链实现自动化 CI/CD �?

- •�?*多智能体协作**：可调用多个专业 AI 智能体（如代码审查员、文档生成器）协同工�?�?

### �?5. **常见问题**
- •�?*安装失败**：检�?Node.js 版本（需 v20+）和网络连接 �?

- •�?*认证失败**：确�?API 密钥无误且无多余空格，或重新生成密钥 �?

- •�?*命令无响�?*：按 `Ctrl+C` 中断后，用 `/clear` 清空上下文再重启 �?
