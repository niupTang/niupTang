---
title: "GitHub 5K Star: 爱盘开源网盘搜索平台，完美支持批量导入导出与在线播放！"
permalink: "github-5k-star-爱盘开源网盘搜索平�?完美支持批量导入导出与在线播�?"
date: 2024-11-26 18:58:11
categories:
  - 开源项目
tags:
  - GitHub
  - 开源项目
cleanup_status: needs_review
---

- 大家好，我是牛皮糖！相信很多人都有多个网盘账户，存储了大量的资源。然而，面对海量的网盘文件，如何快速有效地管理和搜索变得尤为重要。最近，我发现了一个非常实用的开源项目—�?

**爱盘-网盘资源搜索**，它不仅支持多源聚合搜索，还具备强大的在线播放和管理功能，简直是网盘资源管理的利器！

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrR7zQEHVlo8uvLxEz4p69smkkLB21amcKlBiba1E9mNFXlGNkdHZ6SGXWT2dTyicjI5R5pYOyTFYbcA/640?wx_fmt=png&from=appmsg)

### 主要功能
- •�?*🎯 多源聚合搜索**
爱盘支持多个网盘源的资源聚合搜索，你可以一次性搜索多个平台的资源，极大提高了搜索效率�?

- •�?*📺 在线视频播放**
直接在平台上播放网盘中的视频资源，免去下载烦恼，流畅观看�?

- •�?*🗄�?Alist 源聚�?*
集成�?Alist，进一步增强了资源搜索的广度和深度�?

- •�?*📝 博客系统**
爱盘还包含了博客系统，可以进行内容发布和管理�?

- •�?*🔐 后台管理系统**
强大的后台管理系统，支持资源管理、用户管理等功能，让管理更加高效�?

- •�?*📊 资源管理与批量导入导�?*
支持对网盘资源的批量导入和导出，无论�?CSV 还是 XLSX 格式，轻松处理�?

### 部署方式
如果你也想体验这个开源项目，部署起来非常简单。你可以选择以下两种方式�?

- 1. **Vercel 部署（推荐）**
点击项目页面上的 "Deploy with Vercel" 按钮，自动一键部署到 Vercel 平台，非常方便�?

- 2. **Docker 部署（推荐）**
如果你更习惯使用 Docker 部署，爱盘也提供了完整的 Docker 部署文档。只需要执行以下命令即可启动服务：`cp .env.example .env
docker compose up -d`

- 

### 开发指�?
如果你打算参与开发，可以按照以下步骤进行�?

- 1. **安装依赖**
使用 pnpm 包管理器安装项目依赖：`pnpm install`

- 2. **配置环境变量**
根据 `.env.example` 文件配置环境变量：```
`cp .env.example .env`
```

- 3. **数据库设�?*
生成 Prisma 客户端并执行数据库迁移：```
`npx prisma generate
npx prisma migrate deploy`
```

- 4. **启动开发服务器**
启动本地开发服务器：```
`pnpm run dev`
```

![](https://mmbiz.qpic.cn/mmbiz_png/p1ESIQQvfrR7zQEHVlo8uvLxEz4p69smrIYT4GFvS4yCuZytWibDyEFl7GKyyWuES9WT7386ib7AUcFbw08bGW8w/640?wx_fmt=png&from=appmsg)

如果你对爱盘感兴趣，欢迎访问 爱盘-网盘资源搜索，并贡献你的力量�?

项目地址�?

https://github.com/unilei/aipan-netdisk-search

![](https://mmbiz.qpic.cn/mmbiz_gif/7ibzJsmgW5wguO21SlkBAdxJgAicEOVCzDiaObyzEAEMTI527clib7gHvKfBtDu8MJZLwwEIVuVBmqfn01fmLDdTfQ/640?wx_fmt=gif&tp=webp&wxfrom=5&wx_lazy=1)

·················END·················

### **推荐阅读**
�?  [github 95.5k Star 的项目集合地](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485137&idx=1&sn=00da6656ecacfcab683c6e149c208985&chksm=c1d3a4d7f6a42dc107aa9233b7a23f20a4bfe7bcd9d48a0fe24787faae37539bd65663d8ba8e&scene=21#wechat_redirect)[‍](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)�?  [4�?16G 就能 RAGFlow Quick start 快速入门](http://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247485121&idx=1&sn=97093dfe7da78fb786bb999a284ee1fc&chksm=c1d3a4c7f6a42dd1df4cb4de4c057671d57274480eac57e61b4f6bae86aef03ff26bf23ffdd6&scene=21#wechat_redirect)�?  [github 7.8k star 将小爱音箱接�?ChatGPT 和豆包，改造成你的专属语音助手。](http://mp.weixin.qq.com/s?__biz=MzIxODg1OTk1MA==&mid=2247488494&idx=1&sn=0244c0a45012f5a6ca6cdf9a0ac88024&chksm=97e5432fa092ca39991868bda959cfd8bafd949fa445ef72bdca5cc6c4dc2ed4bd51e22634f8&scene=21#wechat_redirect)